...
# 租房
ershoufang_rdd = sc.textFile("hdfs://localhost:9000/house/zufang_price.txt")
...
pair_rdd = ershoufang_rdd.map(lambda x: group(x, province_list)).reduceByKey(
    lambda x, y: x + y).map(lambda x: Row(city=x[0], count=x[1]))
...
schema_ershoufang.write.jdbc(MysqlConfig.MYSQL_CONN, 'every_regions_zufang_count', 'overwrite', conn_param)
print("执行完毕")
